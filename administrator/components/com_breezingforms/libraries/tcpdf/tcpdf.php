...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$signature = $tmparr[1];
			// decode signature
			$signature = CrBcHelpers::bSixtyFourDecode(trim($signature));
		...
		switch($dest) {
			...
			case 'E': {
				...
				$retval .= ' name="'.$name.'"'."\r\n";
				$retval .= 'Content-Transfer-Encoding: '.$ba.$se.$sixtyfour."\r\n";
				$retval .= 'Content-Disposition: attachment;'."\r\n";
				$retval .= ' filename="'.$name.'"'."\r\n\r\n";
				$retval .= chunk_split(CrBcHelpers::bSixtyFourEncode($this->getBuffer()), 76, "\r\n");
				return $retval;
			}
			case 'S': {
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				unset($tmparr);
				// decode signature
				$signature = CrBcHelpers::bSixtyFourDecode($signature);
				// convert signature to hex
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src'][0] === '@') {
						// data stream
						$tag['attribute']['src'] = '@'.CrBcHelpers::bSixtyFourDecode(substr($tag['attribute']['src'], 1));
						$type = '';
					} else {
						// get image type
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h);
                                        ...
                                        $se = 'se';
                                        $sixtyfour = '64';
					if (preg_match('/^data:image\/[^;]+;'.$ba.$se.$sixtyfour.',/', $img, $m) > 0) {
						// embedded image encoded as b64
						$img = '@'.CrBcHelpers::bSixtyFourDecode(substr($img, strlen($m[0])));
					} else {
						// fix image path
