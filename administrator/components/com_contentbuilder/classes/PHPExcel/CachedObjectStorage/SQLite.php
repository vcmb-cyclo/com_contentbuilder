...
class PHPExcel_CachedObjectStorage_SQLite extends PHPExcel_CachedObjectStorage_CacheBase implements PHPExcel_CachedObjectStorage_ICache
{
    ...
    protected function storeData()
    {
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!$this->DBHandle->executeExec("INSERT OR REPLACE INTO kvp_".$this->TableName." VALUES('".$this->currentObjectID."','".sqlite_escape_string(serialize($this->currentObject))."')")) {
    ...
    public function getCacheData($pCoord)
    {
        ...
        $cellResultSet = $this->DBHandle->execute($query, SQLITE_ASSOC);
    ...
    public function deleteCacheData($pCoord)
    {
        ...
        if (!$this->DBHandle->executeExec($query)) {
            throw new PHPExcel_Exception(sqlite_error_string($this->DBHandle->lastError()));
        }
    ...
    public function copyCellCollection(PHPExcel_Worksheet $parent)
    {
        ...
        if (!$this->DBHandle->executeExec('CREATE TABLE kvp_'.$tableName.' (id VARCHAR(12) PRIMARY KEY, value BLOB)
    ...
    public function __construct(PHPExcel_Worksheet $parent)
    {
        ...
        if (is_null($this->DBHandle)) {
            ...
            if (!$this->DBHandle->executeExec('CREATE TABLE kvp_'.$this->TableName.' (id VARCHAR(12) PRIMARY KEY, value BLOB)')) {
                throw new PHPExcel_Exception(sqlite_error_string($this->DBHandle->lastError()));
            }
        }
    }
    ...
    {
        if (!is_null($this->DBHandle)) {
            $this->DBHandle->executeExec('DROP TABLE kvp_'.$this->TableName);
        }
        $this->DBHandle = null;
    }
